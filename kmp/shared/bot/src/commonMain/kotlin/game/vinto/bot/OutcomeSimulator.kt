 ...
 * `packages/bot/src/lib/mcts-outcome-simulator.ts`.
 *
...
object OutcomeSimulator {
     ...
     */
    fun simulateDiscardOutcome(drawnCard: Card, botPlayer: PlayerState): TurnOutcome {
        val afterTossIn = simulateTossInCascade(
            discardedRank = drawnCard.rank,
            currentHandSize = botPlayer.cards.size,
            currentScore = calculateHandScore(botPlayer.cards),
            botPlayer = botPlayer,
        )
    ...
    fun simulateTurnOutcome(
        ...
        val discardedCard = botPlayer.cards.getOrNull(swapPosition)
            ?: return simulateDiscardOutcome(drawnCard, botPlayer)
        ...
        val scoreAfterSwap =
            calculateHandScore(botPlayer.cards) - discardedCard.value + drawnCard.value

        // Stage 2 — the toss-in window the discarded card opens.
        val afterTossIn = simulateTossInCascade(
        ...
        val knowledgeGain = simulateActionKnowledgeGain(discardedCard, botPlayer, context)
    ...
    data class TossInResult(val handSize: Int, val score: Int)

    /**
     * What the bot itself would shed into a window on this rank.
     *
     * Only cards it has actually seen: a card it has not read cannot be tossed in, because
    ...
    fun simulateTossInCascade(
        ...
        for (position in botPlayer.cards.indices) {
            if (position !in botPlayer.knownCardPositions) continue
            val card = botPlayer.cards[position]
    ...
    fun simulateActionKnowledgeGain(
        ...
        return when (discardedCard.rank) {
            ...
            Rank.KING -> calculateKingActionValue(botPlayer)
            Rank.ACE -> ACE_VALUE
            else -> 0
        }
    }
     ...
     * known action card is something to declare — the value is in the hand around it.
     */
    private fun calculateKingActionValue(botPlayer: PlayerState): Int {
        val knownCards = botPlayer.knownCardPositions.mapNotNull { botPlayer.cards.getOrNull(it) }

        val otherKings = knownCards.count { it.rank == Rank.KING }
        val declarableActions = knownCards.count { it.actionText != null && it.rank != Rank.KING }
     ...
     *
     * It needs both halves: a blind spot of the bot's own to fill, and a card it has read
     * somewhere else to fill it with. Either alone is worth nothing.
     */
    private fun simulateKnowledgeGainingSwap(
        ...
        val knowsAnOpponentCard = context.opponentKnowledge.values.any { it.isNotEmpty() }
