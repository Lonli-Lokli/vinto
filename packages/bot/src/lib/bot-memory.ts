...
import { Card, Difficulty, Rank } from '@vinto/shapes';
...
export class BotMemory {
  ...
  observeCard(card: Card, playerId: string, position: number): void {
    ...
    if (!isCorrect) {
      ...
      console.log(
        `[BotMemory] ${this.botId} failed to remember card at ${playerId}[${position}] (random: ${randomValue.toFixed(3)}, threshold: ${this.config.memoryAccuracy})`
      );
      return;
    }
