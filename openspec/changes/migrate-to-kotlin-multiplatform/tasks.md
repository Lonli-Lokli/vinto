...
- [ ] 5.6 **Rewritten — the TypeScript baseline is going away.** Bot strength is now measured against *itself*: a seeded self-play tournament across difficulties, reporting aggregate final scores, coalition win rate and decision latency, with the numbers committed so a later change can be compared to them. The original task compared against a TypeScript baseline generated by `npm run recordings:generate`, which stops existing when `legacy-web/` is deleted (§1d). Not a release blocker either way — 5.5 (legality) is the gate, this is quality
...
- [ ] 6.7 **Rewritten — the TypeScript replayer is going away.** Kotlin self-play recording generator, and a CI job that replays its fresh recordings through the Kotlin engine on **JVM, Kotlin/JS and wasmJs** — the round trip is now across targets rather than across languages, which is the property that still matters once one engine ships (a `Long` is two `Int`s on JS). The original spelled `tools/replay-recording.ts`, which goes with `legacy-web/` (§1d)
...
- [~] 8.2 Crash reporting in `composeApp` — **built, hand-rolled instead of the SDK**, with the reasoning and the bundle measurement in `add-live-analytics/design.md` §A9. The SDK does publish for all four of our targets (checked first, not assumed) and it does catch native crashes a `try`/`catch` cannot; it loses anyway because the web client has no native crash to catch — a Wasm trap is an ordinary JS exception — and the web bundle is the one target with no headroom. Flagged for review rather than settled: `installCrashHandler` is an `expect`/`actual`, so adopting the SDK on Android and iOS alone stays a two-file change.
      ...
      **Not ticked**, and the two reasons are honest ones: the `breadcrumbs (game id + action index)` half of this task is not built — the reporter carries a coarse surface and nothing per-action — and the privacy manifest / permissions review needs Xcode. The DSN is empty in the source; DEPLOYMENT.md §7a says where a release build sets it
