...
- [x] 2.1 Gradle workspace (Kotlin 2.1, KMP plugin, Compose Multiplatform, version catalog): **done, and it is the repository root** rather than `kmp/`. Built under `kmp/` while the web client was still the product; hoisted when it stopped being — `./gradlew` runs from the root, `fixtures/` is a sibling of `shared/`, and the Next.js workspace is retired to `legacy-web/` (design D1, `docs/kotlin/README.md` §1a). Modules present: `shared:shapes`, `shared:engine`, `shared:bot`, `shared:client`, `shared:protocol`, `shared:room`, `worker`, `composeApp`, plus the `iosApp` Xcode project. **Two from the plan do not exist and were not forgotten**: `shared:recording` (the recording model lives in `shared:shapes`, beside the canonical JSON and hashing it depends on — deviation recorded under 3.4) and `parity-tests` (the corpus suites live in the module they gate, so a failure names the engine rather than a test project). `shared:room` was added, and is not in the plan: the room's rules moved out of the Worker so the JVM could test them
- [x] 2.2 Targets: **done** — `androidTarget`, `iosArm64`, `iosSimulatorArm64` (both behind a host check, since Kotlin/Native cannot build Apple targets off macOS), `js(IR)` for the Worker, `wasmJs` for Compose web, and `jvm` for tests and tooling only. Libraries: kotlinx.serialization and kotlinx.coroutines throughout, kotlin.test everywhere, coroutines-test where a suspend API is gated. **Not used, and each for a reason rather than an omission**: Ktor client (`RemoteGameSession` speaks to the room over the platform WebSocket — one dependency fewer on every target, and wasmJs has no Ktor engine worth the weight), Koin (task 6.8, and nothing has yet needed a container), kotlinx-datetime (the engine has no clock at all, by design), Turbine (the `StateFlow` assertions read fine with `runTest` and `first()`)
- [x] 2.3 GitHub Actions: **written, run, and green.** `.github/workflows/kmp.yml` carries six checks — `kmp-detekt` (every module and source set, `maxIssues: 0`), `kmp-jvm` (the six shared modules' JVM suites: the corpus replay, the validator's impersonation sweep, the self-play legality gate), `kmp-web` (the same `commonTest` suites on Kotlin/JS and Wasm, plus the Compose web client's own compile, which nothing else covers), `kmp-android` (`assembleDebug` and the Compose suites headless), `kmp-worker` (the Kotlin/JS bundle, twelve room gates, the gzipped size budget) and `kmp-ios` (simulator tests for the five Apple-target modules and the framework Xcode embeds). Gradle caching through `gradle/actions/setup-gradle`, written from branch pushes and read on pull requests; `~/.konan` cached on the macOS job. Jobs are path-filtered and unchained, the macOS leg is rationed to pushes, nightly, dispatch and `ios`-labelled pull requests, JDK is pinned to 17, wrangler is pinned to an exact version. Two deliberate exclusions, documented where they are made: the golden-screenshot suite (a fresh runner would write its own goldens and pass, asserting nothing) and `nx build @vinto/game` (broken since before this branch, on a frozen workspace).
      **What it found, which is the argument for it existing.** Seven pre-existing detekt findings, reproduced against the tree *before* the Gradle move and baselined rather than fixed blind. Three room-gate assertions the room had outgrown. Two source sets that had never been compiled anywhere — `:composeApp:jvmTestClasses` read a string that was not in `strings.xml`, and `Net.ios.kt` had five import-shaped Kotlin/Native errors that only a compiler catches. Later, `Storage.wasmJs.kt` reaching for `kotlin.js.Date`, which is absent from the Wasm stdlib, so the Compose web client had never compiled at all — and no `index.html`, so the distribution could not have been served. Every one of those is a defect a non-Mac, non-androidx container could not have found. Four of the Compose suite's 73 tests then failed on their first run anywhere; all four are fixed (§1c keeps the diagnosis, because three were not what the log suggested)
...
- [x] 2a.1 Kotlin/JS bundle containing `shared/engine`: **done** — the Worker now carries the real engine and measures **186 KB gzipped**, ~6% of the 3 MB limit. All 50 recordings replay through it in workerd via `POST /replay`, closing the Kotlin/JS-versus-JVM risk. Engine cost is ~0.9 ms per action warm (local `wrangler dev`, wide spread — treat as an order of magnitude, not a benchmark), against a 30 s Durable Object budget **per request**; a whole 278-action game replays in ~250 ms. **MCTS is measured now** (2a.1b, `PLATFORM-GATE.md`): three seeded games through the real room functions, 141 requests and 459 bot actions at `moderate` — mean 131 ms, p50 **4 ms**, p95 590 ms, worst **1,591 ms**, so the worst request observed uses ~5% of a Durable Object's 30 s budget. The median is nothing at all, because the bots only move when the turn reaches them; the tail is one request paying for three searches. `hard` is 5,000 iterations rather than 2,000, so its tail is roughly 2.5x — still inside, and the setting to watch
...
- [x] 6.1 `GameSession` interface and `SessionEvent`: **done** in the new `shared/client`, with
      ...
      `visualView`/`syncVisualState()` were **not** ported — the TypeScript animation seam, deferred
      as a phase 7 question rather than ported blind. **Phase 7 answered it, and the answer was
      no**: the seam is `frames`, a `SharedFlow<List<Frame>>` on the session, one frame per move
      carrying the view that move left behind, walked by `CardStage` at the pace the player
      chose. A second *view* would have been a second source of truth for the same table; a
      list of frames is a log the screen consumes and forgets. `SessionEvent` covers the rest
- [x] 6.2 `LocalGameSession`: **engine + bots in-process, done and gated** — one human against
      ...
      Object does, from the same `GameAction.actorId`. `Recorder` is built in and always on (6.4), so a bug report from a
      phone is a recording the replay harness plays back rather than a paragraph
...
- [~] 7.1 Prototype gate: **decided — an overlay layer**, and it is built and running on an
      ...
      hands, and animating it in place means making room for it in every one of them. **Blocked on hardware for the rest** (README §1f): it has never
      been run on a physical phone, and on Apple it is compiled and simulator-tested by
      `kmp-ios` but not looked at by a person. Nothing about the decision is outstanding
...
- [x] 7.3 Screens/navigation: opening, home, settings, the lesson, the game table, and an
      ...
      other three ways in are under it. **Online is a button that plays.** It was a dialog
      explaining what existed and what did not, back when this app's half was the missing one;
      phase 9 built that half, and it now opens the real thing — a name, a room code, a
      public/private choice, and a browser for the rooms that chose to be listed (§6j). Settings are four
      ...
      `expect`/`actual` `SystemBack`; without it, back from the settings closed the app. **The lobby is built** (`OnlineScreen`, `DiscoverScreen`,
      `RoomScreen`, §6j), which was the one thing this task was waiting on. **The debug screens
      are dropped rather than deferred**, and that is a decision: what they were for is served
      better by the bug-report control, which exports a whole `Recording` the replay harness
      plays back — a screen showing internal state tells you what somebody managed to read off
      it, and a recording tells you the game
...
- [~] 8.1 Android: signing, Play internal track from CI on tags; iOS: bundle id, TestFlight from CI.
      **The sideload half is done**: `assembleRelease` signs with the upload key named by
      ...
      `docs/kotlin/README.md` §6f. **Still open, and the reason has changed**: CI exists now
      and is green on six checks (2.3), so a release job is no longer blocked on there being no
      Gradle job — it is blocked on the secrets a release job would use. An upload key, a Play
      track, R8, and everything iOS: all of it in README §1f, none of it a decision
- [~] 8.2 Crash reporting in `composeApp` — **built, hand-rolled instead of the SDK**, with the reasoning and the bundle measurement in `add-live-analytics/design.md` §A9. The SDK does publish for all four of our targets (checked first, not assumed) and it does catch native crashes a `try`/`catch` cannot; it loses anyway because the web client has no native crash to catch — a Wasm trap is an ordinary JS exception — and the web bundle is the one target with no headroom. Flagged for review rather than settled: `installCrashHandler` is an `expect`/`actual`, so adopting the SDK on Android and iOS alone stays a two-file change.
      ...
      **A gap this did not close, found when an APK crashed on the launcher:** the client cannot report a *startup* crash. `installCrashHandler` runs from a `LaunchedEffect` inside `ReportCrashes`, which is inside `App()`, so anything throwing before or during the first composition happens before the handler exists — and `SENTRY_DSN` is empty in source, so `enabled` is false and it is never installed at all in a debug build. Fixable in two small pieces (install it in `MainActivity.onCreate` before `setContent`; set the DSN for release builds, DEPLOYMENT.md §7a) and not done.
...
- [~] 9.9 **Three of four done; the fourth needs a deployment.** Two clauses were written for an architecture that was not built: there is no **Sentry JVM** and no **container** — the room is a Durable Object, decided in design D1/D9, and the provider was decided with it. Read as "the server reports, and its reports are useful", which is what landed.
      ...
      **Load test with 100 concurrent rooms: no longer blocked on the deployment.** The room is deployed and open (README §6q), so the thing it was waiting for exists. What remains is a decision rather than a dependency: a load test drives a live service that people can now be playing on, and choosing when to do that is not something a build should decide. `wrangler dev` is still not an option — it enforces no CPU limit whatsoever (§6d), so a load test against it would measure the laptop
- [ ] 9.10 **BLOCKED** (README §1f), and on three things now rather than four: an upload key, Play and App Store accounts, and a signed build. The fourth — "a room that is actually open" — is **done**: the Worker is deployed with `ROOM_OPEN` flipped (§6i step 4, recorded in §6q), verified against the live edge by `gate-engine-replay` (50/50) and `gate-two-clients` (sockets, hibernation, reconnect). What is built: `assembleRelease` signs with the upload key when `keystore.properties` exists and with the debug key when it does not, so the signing path is exercised without the secret
