...
**States**: `LOBBY → PLAYING ⇄ BETWEEN_ROUNDS → FINISHED`, then deletion.
...
- Should a public room start automatically when four humans are seated, or wait for the host?
  Auto-start is friendlier for strangers; host-start is friendlier for friends.
- Does a session end on a round count, a wall-clock limit (the rules say ~30 minutes), or
  when players stop agreeing to another round? A wall clock in a room is a scheduling
  question, not an engine one, so it does not threaten determinism — but it needs a decision.
