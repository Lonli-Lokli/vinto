...
- [~] 1.1 **Carried to `ship-and-operate` 4.1.** Blocked on the Cloudflare dashboard for the account that owns the Worker — not on anything in this change, which is why it moved rather than kept this one open. Original note follows. **BLOCKED** (§1f): needs the Cloudflare dashboard for this account. Confirm the account's **current** Workers Analytics Engine allowances — writes/day,
...
- [~] 5.1 **Carried to `ship-and-operate` 4.4.** Blocked on three secrets, addable from a phone, and some traffic to read — not on anything in this change, which is why it moved rather than kept this one open. Original note follows. **BUILT, NOT TICKED** (§1f): `GET /counts?key=…` on the Worker, rendered server-side from the WAE SQL API, with `ANALYTICS_TOKEN`, `ANALYTICS_ACCOUNT_ID` and `DASHBOARD_KEY` as secrets. Absent-safe: missing any of the three answers 404, the same answer a path that does not exist gets, so a prober cannot tell the two apart. `gate-dashboard.mjs` (51 checks) covers the refusals, the escaping and the queries' shape; it cannot cover a single number, because the Analytics Engine SQL API is the one part of WAE `wrangler dev` does not emulate. **The deployment now exists** (README §6q) — the room is live and open — so what is left is the three secrets and some traffic to read. The secrets can be added from a phone through the Cloudflare dashboard rather than `wrangler secret put`; DEPLOYMENT.md §7 says where
...
- [~] 5.3 **Carried to `ship-and-operate` 4.3.** Blocked on the Cloudflare dashboard — and its premise was stale, see 4.2 there — not on anything in this change, which is why it moved rather than kept this one open. Original note follows. **BLOCKED** (§1f): a Cloudflare dashboard toggle, not code. For a Pages project, Web Analytics is enabled per-site in the dashboard and Cloudflare injects the beacon itself — there is nothing in this repository to change, and nothing here can be verified without the account. DEPLOYMENT.md §7b says where the toggle is. What *was* missing and is now fixed is the page it would be injected into: `wasmJsBrowserDistribution` produced two `.wasm` files, a `.js` and **no `index.html`**, so the web client compiled and could not be served at all
- [~] 5.4 **Carried to `ship-and-operate` 4.5.** Blocked on a week of real traffic, which can accrue now the room is open — not on anything in this change, which is why it moved rather than kept this one open. Original note follows. **BLOCKED** on time rather than on a dependency now: the deployed room exists and is open (README §6q), so the week of real traffic this needs can start accumulating. Once a week of real traffic exists: revisit the sampling rates in §A8 against actual
