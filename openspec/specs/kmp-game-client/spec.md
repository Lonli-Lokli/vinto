<!--
  ...
  The `GameSession` seam: one interface with two lives, so a screen cannot tell a solo game
  from an online one.
...
What a screen is allowed to know and allowed to do. A client holds no rules and no second
copy of the game: it reads a redacted view of one seat, dispatches actions, and draws what
the table would show — the same interface whether the engine is in the process or across a
socket.
