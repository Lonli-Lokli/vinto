...
import { GameStore, Difficulty, TossInTime, Card, Rank } from '../shapes';
...
export const useGameStore = create<GameStore>()(
  devtools(
    immer((set, get) => ({
          ...
          state.players.forEach((player) => {
            if (!player.isHuman) {
              ...
              const newKnownSet = new Set(
                Array.from({ length: player.cards.length }, (_, i) => i).filter(
                  () => Math.random() < aiKnowledge
                )
              );
      ...
      updateTossInTime: (newTossInTime: TossInTime) =>
        set((state) => {
        ...
        set((state) => {
          ...
          switch (card.rank) {
            ...
            case 'Q':
              ...
              break;
            case 'K':
              // "Declare the value of any card and play its action"
              state.actionContext = {
                action: card.action,
                playerId: playerId,
        ...
        set((state) => {
          ...
          if (targetType === 'opponent-card' && targetPlayerId !== playerId) {
            if (position >= 0 && position < targetPlayer.cards.length) {
              ...
              GameToastService.success(
                `${actionPlayer.name} peeked at ${
                  targetPlayer.name
                }'s position ${position + 1}: ${peekedCard.rank} (value ${
                  peekedCard.value
                })`
              );
            }
          ...
          if (targetType === 'peek-then-swap') {
            // Check if this card is already selected (to allow deselection)
            const existingIndex = state.peekTargets.findIndex(
              (target) =>
                target.playerId === targetPlayerId &&
                target.position === position
            );
            ...
            if (existingIndex !== -1) {
              // Card is already selected, remove it (deselect)
              state.peekTargets.splice(existingIndex, 1);
              GameToastService.info(
                `${actionPlayer.name} deselected ${targetPlayer.name}'s card ${
                  position + 1
                }`
              );
              return; // Don't continue with peek logic
            }

            // Don't allow more than 2 selections
            if (state.peekTargets.length >= 2) {
              GameToastService.info(
                'Already have 2 cards selected. Deselect one to choose a different card.'
              );
              return;
            ...
            if (position >= 0 && position < targetPlayer.cards.length) {
              ...
              state.peekTargets.push(newTarget);
              ...
              if (state.peekTargets.length === 2) {
                const [peek1, peek2] = state.peekTargets;
                GameToastService.info(
                  `Cards peeked: ${peek1.card!.rank} (${
                    peek1.card!.value
                  }) and ${peek2.card!.rank} (${
                    peek2.card!.value
                  }). Choose to swap them or skip.`
                );
          ...
          if (targetType === 'force-draw') {
            ...
            if (targetPlayerId !== playerId && state.drawPile.length > 0) {
      ...
      // Execute Queen's optional swap
      executeQueenSwap: () =>
        set((state) => {
          if (state.peekTargets.length !== 2) return;

          const [target1, target2] = state.peekTargets;
      ...
      // Skip Queen's optional swap
      skipQueenSwap: () =>
        set((state) => {
          GameToastService.info(
            'Queen action: Chose not to swap the peeked cards'
          );
      ...
      // Declare King action - choose any card's action to execute
      declareKingAction: (rank: Rank) =>
        set((state) => {
          if (
            !state.actionContext ||
            state.actionContext.targetType !== 'declare-action'
          )
            return;

          const { playerId } = state.actionContext;
          const actionPlayer = state.players.find((p) => p.id === playerId);
          if (!actionPlayer) return;

          // Get the action for the declared rank
          const getActionForRank = (rank: Rank): string | undefined => {
            switch (rank) {
              case '2':
              case '3':
              case '4':
              ...
              case 'K':
                return "Declare any card's action"; // Recursive K is allowed
              case 'A':
          ...
          const declaredAction = getActionForRank(rank);

          if (!declaredAction) {
            GameToastService.info(`${rank} has no action to declare`);

            // Clean up and advance turn since there's no action to execute
            state.isAwaitingActionTarget = false;
            state.actionContext = null;
          ...
          state.actionContext.declaredCard = rank;

          // Execute the declared card's action by calling executeCardAction recursively
          // But first we need to update the context to reflect the new action type
          if (rank === '7' || rank === '8') {
            state.actionContext.targetType = 'own-card';
          } else if (rank === '9' || rank === '10') {
            state.actionContext.targetType = 'opponent-card';
          } else if (rank === 'J') {
            state.actionContext.targetType = 'swap-cards';
          } else if (rank === 'Q') {
            state.actionContext.targetType = 'peek-then-swap';
          } else if (rank === 'K') {
            // Recursive King declaration - keep declare-action type
            state.actionContext.targetType = 'declare-action';
          } else if (rank === 'A') {
            state.actionContext.targetType = 'force-draw';
          }

          state.actionContext.action = declaredAction;
      ...
      cancelAction: () =>
        set((state) => {
          ...
          state.peekTargets = []; // Clear any partially selected peek targets
      ...
      tossInCard: (playerId: string, position: number) =>
        set((state) => {
          ...
          if (tossedCard && tossedCard.rank === topDiscard.rank) {
            ...
            GameToastService.success(
              `${player.name} tossed in ${tossedCard.rank}!`
            );
          } else {
          ...
          set((draft) => {
            ...
            setTimeout(() => {
              set((draftTimeout) => {
                ...
                draftTimeout.currentPlayerIndex =
                  (draftTimeout.currentPlayerIndex + 1) %
                  draftTimeout.players.length;
                // Scoring begins only after a called Vinto final round completes
                if (
                  draftTimeout.finalTurnTriggered &&
                  draftTimeout.currentPlayerIndex === 0
                ) {
      ...
      calculateFinalScores: () => {
        ...
        state.players.forEach((player) => {
          scores[player.id] = player.cards.reduce(
