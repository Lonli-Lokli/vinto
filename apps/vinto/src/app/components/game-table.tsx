...
export const GameTable = observer(() => {
  ...
  const handleCardClick = (position: number) => {
    ...
    if (isSelectingSwapPosition) {
      gameClient.dispatch(GameActions.swapCard(humanPlayer.id, position));
  ...
  const getPlayerPosition = (player: PlayerState): PlayerPosition => {
    ...
    const playerCount = gameClient.state.players.length;

    // For 2 players: human (bottom), opponent (top)
    if (playerCount === 2) return 'top';

    // For 3 players: human (bottom), opponent1 (top), opponent2 (left)
    if (playerCount === 3) {
      return playerIndex === 1 ? 'top' : 'left';
    }

    // For 4 players: human (bottom), opponent1 (top), opponent2 (left), opponent3 (right)
