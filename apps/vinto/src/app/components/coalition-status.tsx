      ...
      <div className="bg-surface-secondary/95 backdrop-blur-sm rounded-lg shadow-lg mx-2 my-1 px-3 py-2.5 border border-primary/20">
        ...
        <div className="flex items-center justify-between mb-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-primary font-bold text-sm uppercase tracking-wide">Final Round</span>
          </div>
          {currentPlayer && (
            <div className="text-secondary text-sm">
              <span className="font-semibold text-primary">{currentPlayer.name}</span>
              {!isCurrentPlayerLeader && !isCurrentPlayerVinto && currentPlayer.id !== coalitionLeader.id && (
                <span className="text-tertiary text-xs ml-1">(Leader decides)</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-start">
          ...
          <div className="space-y-1.5">
            <div className="text-warning text-xs font-semibold uppercase tracking-wide mb-1.5">Coalition</div>
            ...
            <div className="flex items-center gap-2 bg-primary/5 rounded px-2 py-1.5 border-l-2 border-warning">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-primary text-sm font-semibold truncate">{coalitionLeader.name}</div>
            ...
            {coalitionMembers
              ...
              .map((member) => (
                ...
                >
                  ...
                  <div className="text-primary text-sm truncate">{member.name}</div>
                </div>
              ))}
          </div>
          ...
          {/* Vinto */}
          <div className="space-y-1.5">
            <div className="text-error text-xs font-semibold uppercase tracking-wide mb-1.5">Vinto Caller</div>
            ...
            {vintoCaller && (
              <div className="flex items-center gap-2 bg-error/10 rounded px-2 py-1.5 border-l-2 border-error">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-primary text-sm font-semibold truncate">{vintoCaller.name}</div>
