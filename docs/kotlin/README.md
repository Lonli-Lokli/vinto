...
| ~~analytics 5.1 — dashboard route~~ | **No longer blocked, and no longer a task.** | The route and its three secrets are **gone** (cffc972): `GET /counts?key=…` and `gate-dashboard.mjs` were a per-game dashboard with per-game SQL that every other game would have needed a copy of. Counts go to the shared `kupalinka_events` dataset and are drawn at `stats.kupalinka.app`, which finds this game through `kupalinka.app/games.json` and compiles the `stats.json` the website publishes. Account-scoped datasets mean the room's binding is the whole connection — no token, no account id, no password. `ship-and-operate` 4.4 and `DEPLOYMENT.md` §7 carry it |
...
| ROOM.md §6i step 1 — the ten goldens | A human looking at the images | `ScreenshotTest` writes them and CI deliberately does not run it: a fresh runner would write its own and assert nothing. The PNGs **are** committed, so a change to a screen turns up in a review as a picture — what is still needed is somebody to look at it |
