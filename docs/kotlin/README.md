...
what went is the ability to *regenerate* it, which is the point of §1d.
...
| Settings — a language selector | One translated `strings.xml` | The app has `values/` and nothing else. §6h's eight slices existed so that adding a language would be a file and no code, and no file has been added — so a selector today is a control with a single option in it, which is worse than no control. The unblocking step is a translation, not screen work |
...
baseline generated by `npm run recordings:generate`, which went with `legacy-web/` (§1d).
